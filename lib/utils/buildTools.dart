...
Future<GigInfo> buildGigInfo(gigID) async {
  ...
  Map json = await fetchGigDetails(gigID) as Map<dynamic, dynamic>;
...
List<Gig> buildGigFromJSON(plans) {
  ...
  for (int i = 0; i < plans.length; i++) {
    String? title = plans[i]["title"];
    String date = plans[i]["date"];
    String status = plans[i]["gig_status"].toString();
    // String planValue = plans[i]["plan_status"].toString();
    // String planValueLabel =
    // planValueLabelGenerator(plans[i]["plan_status"].toString());
    // String? planComment = plans[i]["plan"]["comment"];
    // String? planID = plans[i]["plan"]["id"];
    String? gigID = plans[i]["id"].toString();
    // String bandID = plans[i]["gig"]["band"].toString();
    // String? bandShortName = plans[i]["band"]["shortname"];
    String? bandLongName = plans[i]["band"];
...
Future<List<Gig>> buildGigList() async {
  ...
  Map json = await fetchAgenda() as Map<dynamic, dynamic>;
  print('Agenda fetched: ${json.toString()}');
  List upcomingPlans = json["gigs"];

  List<Gig> upcomingPlansList = [];

  upcomingPlansList = buildGigFromJSON(upcomingPlans);

  List<Gig> combinedList = [...upcomingPlansList];
  return combinedList;
}
...
//   List gigMemberList = [];
//   List gigMemberInfo = await (fetchGigMemberInfo(gigID) as List<dynamic>);
...
//     var bandSectionMap = await fetchBandInfo(bandIDs[i]);
//     if (bandSectionMap is Map) {
...
//   var agenda = await fetchAgenda();
//   if (agenda is! Map) return [];
//   //List weighInPlans = agenda["weighin_plans"] ?? [];
//   List upcomingPlans = agenda["gigs"] ?? [];
...
//     String bandID = upcomingPlans[i]["band"].toString();
//     compiledBandIDs.add(bandID);
//   }
